...
#include <stdbool.h>
#include <zephyr/types.h>  // for uint32_t, uint64_t
...
typedef struct {
    uint32_t sample_id;      // Unique sample ID
    uint64_t timestamp_ms;   // Timestamp of data reading
    int16_t x;
    int16_t y;
    int16_t z;
} lis3dh_data_t;
...
// Optional interrupt configuration function (commented out in .c)
// bool lis3dh_set_int_click_config(lis3dh_sensor_t *dev, const lis3dh_int_click_config_t *cfg);

bool lis3dh_read_data(lis3dh_sensor_t *dev, lis3dh_data_t *data);
