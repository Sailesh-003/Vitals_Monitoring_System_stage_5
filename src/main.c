...
int main(void)
{
    ...
    while(1){
        ...
        printk("i2c suspend ret = %d\n", ret);
        ret = pm_device_action_run(flash_dev, PM_DEVICE_ACTION_SUSPEND);
        printk("flash suspend ret = %d\n", ret);

        rtc2_set_alarm(WAKEUP_INTERVAL_MS);
        printk("Going into sleep\n");
