...
// #define ADC_INPUT_POS_TEMP    NRF_SAADC_INPUT_AIN2  // AIN2 = P0.04
...
#define ADC_INPUT_POS_BATT    NRF_SAADC_INPUT_AIN1  // AIN1 = P0.03
...
#define ADC_FULL_SCALE         3600                /* mV at ADC input (0.6V * 6) */
...
/* Battery read */
int read_battery_mv(void) {
    // if (adc_read_channel(ADC_CHANNEL_BATTERY, &adc_sample_batt) != 0) return -1;
    // int32_t mv = adc_sample_batt;
    ...
    int32_t raw = adc_sample_batt;
   
    int mv_at_adc = (raw * ADC_FULL_SCALE) / ADC_MAX_VALUE;   // 600 mV reference × gain 1/6
    int32_t batt_mv = mv_at_adc * DIVIDER_RATIO;                // compensate divider

    printk("Raw=%d, ADC mV=%d, Batt mV=%d\n", raw, mv_at_adc, batt_mv);
    return batt_mv;
}
...
int temp_sensor_init(void) {
    ...
    struct adc_channel_cfg temp_cfg = {
        ...
        .acquisition_time = ADC_ACQ_TIME_CFG,
        .channel_id     = ADC_CHANNEL_TEMP,
        .input_positive = SAADC_CH_PSELP_PSELP_AnalogInput2,//ADC_INPUT_POS_TEMP,
    };
    ...
    struct adc_channel_cfg batt_cfg = {
        ...
        .acquisition_time = ADC_ACQ_TIME_CFG,
        .channel_id     = ADC_CHANNEL_BATT,
        .input_positive = SAADC_CH_PSELP_PSELP_AnalogInput1,//ADC_INPUT_POS_BATT,
    };
